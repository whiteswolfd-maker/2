...
def _hugoniot_pressure_ratio(P_s: np.ndarray, P_a: float) -> np.ndarray:
    """Dimensionless pressure ratio r = P_s / P_a."""
    return np.asarray(P_s, dtype=np.float64) / P_a
...
def post_shock_state(
    P_s: np.ndarray | float,
    gamma: float,
    P_a: float,
    rho_a: float,
) -> dict[str, np.ndarray | float]:
    """Return a dict with all post-shock quantities for pressure P_s.

    Keys: 'D_s', 'u_s', 'rho_s', 'P_s'.
    """
    P_s_arr = np.asarray(P_s, dtype=np.float64)
    D_s = shock_speed(P_s_arr, gamma, P_a, rho_a)
    u_s = shock_velocity(P_s_arr, gamma, P_a, rho_a)
    rho_s = rho_a * rho_ratio(P_s_arr, gamma, P_a)
    return {"D_s": D_s, "u_s": u_s, "rho_s": rho_s, "P_s": P_s_arr}
