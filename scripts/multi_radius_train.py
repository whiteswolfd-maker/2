...
def load_model_and_data(r_mm: int):
    ...
    asn_eval = asn
    if det_pt.exists() and asn_pt.exists():
        det.load_state_dict(torch.load(det_pt, map_location="cpu", weights_only=False)["state_dict"])
        asn.load_state_dict(torch.load(asn_pt, map_location="cpu", weights_only=False)["state_dict"])
        det.eval(); asn.eval()
